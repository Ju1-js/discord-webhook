...
webhook_image_title = "Image Prompt"
        ...
        with gr.Tab(label="Sharing Embed"):
            with gr.Row().style(equal_height=False):
                with gr.Column(variant='panel'):
                    ...
                    _webhook_image_title = gr.Textbox(value=webhook_image_title,interactive=True,label="Title")
            ...
            _webhook_image_title.change(fn=image_save_title, inputs=[_webhook_image_title], outputs=[__output])
...
def generate_image_html():
    ...
    nl = "\n"
    if not webhook_image_prompt:
        webhook_image_description = ""
    return f'<div class="embed_wrapper"><div class="content">{webhook_image_content}</div><div style="border-color: {_webhook_image_color}; max-width: 332px" class="embed_color"><div class="embed_text"><span class="embed_title_wrapper">{webhook_image_title}</span><div class="embed_description_wrapper"><div class="embed_text desc">{webhook_image_description + nl}<a href="{embed_url}" style="color: #328DD2" target="_blank">{embed_url}</a></div></div></div></div></div>'
...
def image_save_embed(input, key):
    global webhook_image_content, webhook_image_title, webhook_image_description, webhook_image_footer, webhook_image_avatar_name, webhook_image_avatar_url, webhook_image_color
    if key == "content":
...
def generate_image_embed():
    global webhook_image_content, webhook_image_title, webhook_image_description, webhook_image_footer, webhook_image_avatar_name, webhook_image_avatar_url, webhook_image_color
    return {"content":webhook_image_content,"embeds":[{"title":webhook_image_title,"url":"","description":webhook_image_description,"color": webhook_image_color,"footer":{"text":webhook_image_footer},"timestamp": datetime.datetime.utcnow().isoformat() + "Z"}],"username":webhook_image_avatar_name,"avatar_url":webhook_avatar_url,"attachments":[]}
